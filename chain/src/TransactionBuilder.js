...
const { Signature, PublicKey, hash } = require('../../ecc');
...
function _broadcast(was_broadcast_callback) {
	return new Promise((resolve, reject) => {
		...
		Apis.instance().networkApi().exec('broadcast_transaction_with_callback', [function (res) {
			return resolve(res);
		}, tr_object]).then(() => {
					...
					for (const pubkey_string of required_pubkeys) {
						if (!signer_pubkeys_added[pubkey_string]) {
							...
							if (!private_key) {
								...
								// returned keys = require(my_pubkeys
