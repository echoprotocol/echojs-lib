...
class API {
		...
		try {
			...
			for (let i = 0; i < length; i += 1) {
				...
				if (isAccountId(key)) {
					...
					this.cache.setInMap(CacheMaps.OBJECTS_BY_VOTE_ID, voteId, new Map(requestedObject))
						.setInMap(CacheMaps.WITNESS_BY_ACCOUNT_ID, accountId, new Map(requestedObject))
						.setInMap(CacheMaps.WITNESS_BY_WITNESS_ID, key, new Map(requestedObject));
					...
					this.cache.setInMap(CacheMaps.OBJECTS_BY_VOTE_ID, voteId, new Map(requestedObject))
						.setInMap(CacheMaps.COMMITTEE_MEMBERS_BY_ACCOUNT_ID, accountId, new Map(requestedObject))
						.setInMap(CacheMaps.COMMITTEE_MEMBERS_BY_COMMITTEE_MEMBER_ID, key, new Map(requestedObject));
	...
	async lookupVoteIds(votes, force = false) {
		...
		for (let i = 0; i < length; i += 1) {
			...
			this.cache.setInMap(CacheMaps.OBJECTS_BY_VOTE_ID, key, new Map(requestedObject))
				.setInMap(CacheMaps.OBJECTS_BY_ID, id, new Map(requestedObject));

			if (requestedObject.committee_member_account) {

				const accountId = requestedObject.committee_member_account;

				this.cache.setInMap(CacheMaps.COMMITTEE_MEMBERS_BY_ACCOUNT_ID, accountId, new Map(requestedObject))
					.setInMap(CacheMaps.COMMITTEE_MEMBERS_BY_COMMITTEE_MEMBER_ID, id, new Map(requestedObject));

			} else if (requestedObject.witness_account) {

				const accountId = requestedObject.witness_account;

				this.cache.setInMap(CacheMaps.WITNESS_BY_ACCOUNT_ID, accountId, new Map(requestedObject))
					.setInMap(CacheMaps.WITNESS_BY_WITNESS_ID, id, new Map(requestedObject));
