...
class Subscriber {
	...
	setSubscribeCallback(callback, notifyRemoveCreate) {
		if (!isFunction(callback)) {
			Promise.reject(new Error('Callback should be a function'));
			return;
		}

		if (!isBoolean(notifyRemoveCreate)) {
			Promise.reject(new Error('notifyRemoveCreate should be a boolean'));
			return;
		}

		this.wsApi.database.setSubscribeCallback(callback, notifyRemoveCreate);
	}
	...
	setPendingTransactionCallback(callback) {
		...
		this.wsApi.database.setPendingTransactionCallback(callback);
	}
	...
	setBlockAppliedCallback(callback) {
		...
		this.wsApi.database.setBlockAppliedCallback(callback);
	}
	...
	subscribeToMarket(callback, baseAssetId, quoteAssetId) {
		if (!isFunction(callback)) {
			Promise.reject(new Error('Callback parameter should be a function'));
			return;
		}
