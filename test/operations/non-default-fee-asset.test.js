...
describe('Non default fee asset', () => {
	...
	before(async function () {
		...
		const { 1: chosenAssetId } = assetCreationRes[0].trx.operation_results[0];
	...
	describe('when should succeed', () => {
		...
		it('required fees calculation should not rejects', async () => tx.setRequiredFees());
		/** @type {RawAssetIssueOperationProps} */
		let operationProps;
		it('transaction should contains operation', () => {
			ok(tx.operations.length > 0);
			const [, txOperationProps] = tx.operations[0];
			operationProps = txOperationProps;
		});
		...
		it('transaction signing should not rejects', async () => tx.sign(privateKey));
		it('transaction broadcasting should not rejects', async () => tx.broadcast()).timeout(15e3);
	});
});
