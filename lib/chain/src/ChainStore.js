...
class ChainStore {
	...
	getBlock(number) {
		...
		let pendingBlock = this.block_requests.get(number);
		...
		pendingBlock.promise = Apis.instance().dbApi()
			.exec('get_block', [number]).then((block) => {
				this.block_requests.delete(number);
				return block;
			});

		this.block_requests.set(number, pendingBlock);
		return pendingBlock.promise;
	}
