...
class TransactionBuilder {
    ...
    */
    ...
    /**
        ...
        @arg {array<string>} [signer_pubkeys = null] - Optional ["GPH1Abc9Def0...", ...].  These are additional signing keys.  Some balance claims require propritary address formats, the witness node can't tell us which ones are needed so they must be passed in.  If the witness node can figure out a signing key (mostly all other transactions), it should not be passed in here.
	...
	set_required_fees(asset_id) {
		...
		return Promise.all(promises).then((results) => {
			...
			let dynamicPromise = (asset_id !== "1.3.0" && asset) ?
				Apis.instance().db_api().exec("get_objects", [[asset.dynamic_asset_data_id]]) :
			...
			.then((dynamicObject) => {
				...
				if (DEBUG) console.log("... get_required_fees", operations, asset_id, flat_assets);
			});
	}
