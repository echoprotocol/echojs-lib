...
class Aes {
	...
	static fromSeed(seed) {
		...
		if (DEBUG) {console.log("... fromSeed _hash", _hash);}
    ...
    */
	static decrypt_with_checksum(private_key, public_key, nonce, message, legacy = false) {
		...
		if (DEBUG) {console.log("... planebuffer", planebuffer);}
    ...
    */
	static encrypt_with_checksum(private_key, public_key, nonce, message) {
		...
		if (DEBUG) {console.log("... S", S.toString("hex"));}
		var checksum = sha256(message).slice(0, 4);
		var payload = Buffer.concat([checksum, message]);
		if (DEBUG) {console.log("... payload", payload.toString());}
